...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        # NOTE: If context dimension is provided, validate that spatial transformer is enabled.
        ...
        # NOTE: Convert num_res_blocks to a list format if it's an integer, else validate its length.
        # NOTE: If user passes in an integer value for number of residual blocks, make it into a list such that
        # e.g., channel_mult = [1 2 4 8] and num_res_blocks = 3 --> [3 3 3 3]
        ...
        self.time_embed = nn.Sequential(
            ...
            nn.SiLU(),                               # NOTE: Activation function SiLU.
        ...
        if self.num_classes is not None:
            if isinstance(self.num_classes, int):
                ...
                self.label_emb = nn.Linear(1, time_embed_dim)   # NOTE: Linear embedding for continuous classes.
            else:
                raise ValueError()
        ...
        self.input_blocks = nn.ModuleList(
            [
                TimestepEmbedSequential(
                    conv_nd(dims, in_channels, model_channels, 3, padding=1)   # NOTE: Initial convolutional layer.
                )
            ]
        )
        ...
        for level, mult in enumerate(channel_mult):
            for nr in range(self.num_res_blocks[level]):
                ...
                # NOTE: Add layers to input_blocks and update feature size.
                self.input_blocks.append(TimestepEmbedSequential(*layers))
                self._feature_size += ch
                input_block_chans.append(ch)

            # NOTE: Add downsampling layer between levels if not at the last level.
        ...
        # NOTE: Handle num_head_channels for middle block attention.
        if num_head_channels == -1:
            dim_head = ch // num_heads
        else:
            num_heads = ch // num_head_channels
        ...
        # NOTE: Initialize middle block with attention or spatial transformer.
        ...
        for level, mult in list(enumerate(channel_mult))[::-1]:
            for i in range(self.num_res_blocks[level] + 1):
                ...
                # NOTE: Add upsampling layer if not at the last level.
                ...
                # NOTE: Add layers to output_blocks and update feature size.
                self.output_blocks.append(TimestepEmbedSequential(*layers))
                self._feature_size += ch

        # NOTE: Initialize final output layer for producing the output Tensor.
        ...
        # NOTE: If predict_codebook_ids is True, initialize ID predictor for discrete IDs.
